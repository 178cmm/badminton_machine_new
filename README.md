...
### 🎮 外接遙控（MVP）
- **簡報筆遙控**：支援 2.4GHz USB 接收器模擬鍵盤輸入
- **一鍵開始/暫停**：空白鍵（Space）切換開始/暫停狀態
- **急停保護**：Esc 鍵緊急停止所有訓練
- **狀態燈顯示**：顯眼的視覺狀態指示（待機/發球中/急停）
- **統一控制入口**：所有來源（按鈕、鍵盤、語音）匯流到同一控制邏輯
- **防抖機制**：500ms 防抖避免重複觸發
- **審計記錄**：所有遙控操作記錄到 `logs/commands.jsonl`
...
1. **硬體準備**
   - 插入 2.4GHz USB 接收器到電腦
   - 確保簡報筆與接收器配對成功
   - 測試簡報筆按鍵是否正常發送鍵盤訊號

2. **軟體設定**
   - 啟動羽毛球發球機控制系統
   - 確保應用視窗保持前景聚焦（QShortcut 限制）
   - 觀察主視窗下方的狀態燈是否顯示「💤 待機」

3. **遙控操作**
   - **開始/暫停**：按簡報筆的「播放/空白鍵」或鍵盤空白鍵
   - **急停**：按簡報筆的「停止鍵」或鍵盤 Esc 鍵
   - **狀態確認**：觀察狀態燈變化
     - 🟡 待機：黃底黑字「💤 待機」
     - 🟢 發球中：綠底白字「🚀 發球中」
     - 🔴 急停：紅底白字「🛑 急停」
...
4. **防抖機制**
   - 系統內建 500ms 防抖，避免簡報筆一次送多個鍵訊
   - 快速連續按鍵只會觸發一次狀態切換

5. **審計記錄**
   - 所有遙控操作自動記錄到 `logs/commands.jsonl`
   - 記錄包含：事件類型、狀態、時間戳、來源（keyboard/ui/voice）

### 注意事項

- **視窗聚焦**：遙控功能需要應用視窗保持前景聚焦
- **相容性**：支援所有標準 2.4GHz USB 簡報筆
- **擴充性**：未來可支援全域熱鍵（需 macOS 輔助使用權限）

### 故障排除

- **遙控無效**：檢查視窗是否聚焦、USB 接收器是否正常
- **狀態燈異常**：重啟應用程式或檢查系統服務狀態
- **記錄缺失**：檢查 `logs/` 目錄權限和磁碟空間
